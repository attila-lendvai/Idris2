...
;;
;; To enter a shell environment with all the dependencies:
;;   guix environment -l guix.scm
;;
;; And an isolated environment some more tools:
;;   guix environment --pure --load=guix.scm --ad-hoc less git:gui
...
(define *source-dir* (canonicalize-path (string-append (dirname (current-filename)) "/..")))
...
(package
  ...
  (native-inputs
   `(("bootstrap-idris" ,idris2-0.1.1)
     ("cc" ,clang-toolchain-12) ; clang-toolchain in guix master is older and doesn't have a bin/cc in its output
     ("coreutils" ,coreutils)
     ("git" ,git)
     ("node" ,node)
     ("sed" ,sed)))
  ...
  (arguments
   `(#:make-flags
     ...
     (modify-phases %standard-phases
       (delete 'bootstrap)
       (delete 'configure)
       (delete 'check) ; check must happen after install
       ...
       (add-after 'wrap-program 'check
         (lambda* (#:key outputs make-flags #:allow-other-keys)
           (apply invoke "make"
                  "INTERACTIVE="
                  (string-append "IDRIS2=" (assoc-ref outputs "out") "/bin/idris2")
                  "test" make-flags))))))
