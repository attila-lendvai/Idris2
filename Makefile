...
#      make distclean
#      make BOOTSTRAP_IDRIS=idris2-0.5.1 IDRIS2_CG=chez stage1 && make BOOTSTRAP_IDRIS=idris2-0.5.1 IDRIS2_CG=racket stage1 && make prepare-bootstrap-files
#
...
define make-stage-generic
  $(MAKE) --makefile Makefile-one-stage.mk \
...
# make clean. Also note that stage0 is only part of the prerequisits
...
bootstrap:
	...
	-test -d .git && git checkout $(BUILD) || true
...
prepare-bootstrap-files:
	sed -i -e 's|$(IDRIS2_CURDIR)/$(STAGE1)/installed|__PREFIX__|' \
	  $(STAGE1)/$(NAME)_app/$(NAME).ss \
	  $(STAGE1)/$(NAME)_app/$(NAME).rkt
	...
	git add --force \
	  ...
	  $(STAGE1)/$(NAME)_app/$(NAME).ss \
	  $(STAGE1)/$(NAME)_app/$(NAME).rkt
	@echo "The bootstrap shortcut files should have been added to the git index, ready to be committed."
...
# handicap.
#
# When this gets fixed then something like the folloing should suffice:
